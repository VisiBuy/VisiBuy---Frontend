...
interface ComboboxOption {
  ...
  isCustom?: boolean; // Flag to identify custom-added options
}
...
interface CustomComboboxProps {
  ...
  multiselect?: boolean; // Enables multiselect mode
  allowCustomOptions?: boolean; // New prop to enable adding custom options
  customOptionLabel?: string; // Label for the "Add custom option" button
  onCustomOptionAdded?: (option: ComboboxOption) => Promise<void> | void; // Optional callback when custom option is added
}
...
export const Combobox: React.FC<CustomComboboxProps> = ({
  ...
  allowCustomOptions = false, // Default to not allowing custom options
  customOptionLabel = "Add custom option", // Default label
  onCustomOptionAdded,
}) => {
  ...
  useEffect(() => {
    // Merge external options with any custom options that were added
    const customOptions = localOptions.filter((opt) => opt.isCustom);
    setLocalOptions([...options, ...customOptions]);
  }, [options]);
  ...
  useEffect(() => {
    if (!multiselect && currentValues) {
      const selectedOption = localOptions.find(
        (option) => option.value === currentValues
      );
      if (selectedOption) {
  ...
  }, [currentValues, localOptions, multiselect]);
  ...
  }, [searchQuery, localOptions, searchable]);
  ...
  const addCustomOption = () => {
    ...
    // Create a unique ID for the custom option
    const timestamp = Date.now();
    const customOption: ComboboxOption = {
      value: searchQuery.trim(), // the custom value searched by the user
      label: searchQuery.trim(), // the custom value searched by the user
      isCustom: true,
    };

    // Add to local options
    setLocalOptions((prev) => [...prev, customOption]);

    // Select the new option
    if (multiselect) {
      const newValues = Array.isArray(currentValues)
        ? [...currentValues, customOption.value]
        : [customOption.value];
      form.setValue(name, newValues, { shouldValidate: true });
    } else {
      setInputValue(customOption.label);
      form.setValue(name, customOption.value, { shouldValidate: true });
      setIsOpen(false);
    }
    ...
    // Call onCustomOptionAdded callback if provided
    if (onCustomOptionAdded) {
      onCustomOptionAdded(customOption);
    }

    // Focus appropriate input
    if (multiselect && searchInputRef.current) {
      searchInputRef.current.focus();
    ...
    switch (e.key) {
      ...
      case "Enter":
        ...
        if (
          ...
          // Add custom option if enabled and there are no matching options
          addCustomOption();
        } else {
          setIsOpen((prev) => !prev);
        }
  ...
  // Check if the search query doesn't match any existing options
  const noExactMatch =
    searchQuery.trim() !== "" &&
  ...
  return (
    <div ref={comboboxRef} className="relative w-full">
      ...
      {isOpen && (
        <div className="absolute z-10 w-full mt-1 bg-white rounded-md shadow-lg border max-h-72 overflow-hidden flex flex-col">
          {searchable && (
          ...
          >
            ...
            {allowCustomOptions && searchQuery.trim() && noExactMatch && (
              <li className="border-t">
                <button
                  type="button"
                  onClick={addCustomOption}
                  className="w-full px-4 py-2 text-blue-600 hover:bg-blue-50 flex items-center"
                >
                  <PlusCircle size={16} className="mr-2" />
                  <span>
                    {customOptionLabel}: "<strong>{searchQuery}</strong>"
                  </span>
                </button>
              </li>
            )}
